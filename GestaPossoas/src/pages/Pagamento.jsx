...
export default function Pagamento() {
  ...
  async function payment(){
      ...
      try{
        const data = {
          user_id: user.id,
          desconto: Number((salario.salario || 0) * (((desconto.faltas * numFaltas || 0) + (desconto.justificadas * numJustificadas || 0) + (desconto.atrasos * numAtrasos || 0) + (desconto.desempenho * numDesemp || 0)) / 100)),
          bonus: (Number(salario.salario) || 0) * (((Number(salario.transporte) || 0) + (Number(salario.alimentacao) || 0) + (Number(salario.presenca) || 0) + (Number(salario.desempenho) || 0)) / 100),
          total: salaryTotal,
          data: day
        };
  
        const response = await api.post('/pagamento/create', data);
        console.log(response.data);
  
        alert('Pagamento Realizado Com Sucesso!');
  
      } catch(error){
        console.error('Erro ao realizar o pagamento:', error?.response?.data || error);
  ...
  return (
    <>
      ...
      <main style={{ padding: '6rem 0' }} className="bg-blue-100">
          ...
          {processamento ? <Processamento text="Confirmando o pagamento..."/> : ''}
            ...
            <div className="mx-auto mb-10 w-8/12 border border-blue-300 rounded-xl bg-white p-6 font-mono shadow-md">
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-2">
                    ...
                    <div className="flex gap-4">
                      <label htmlFor="dat" className="text-3xl font-mono">Data</label>
                      <input type="date" value={day} id="dat" className="border border-gray-600 px-6 h-12 rounded-lg w-7/10"
                      onChange={(e) => setDay(e.target.value)}/>
                    </div>

                  <button className="min-w-[50%] py-4 border border-green-900 bg-green-300 rounded-lg text-xl text-green-900 mt-4" 
                    onClick={() => payment()}>
                    Confirmar Pagamento
                  </button>
