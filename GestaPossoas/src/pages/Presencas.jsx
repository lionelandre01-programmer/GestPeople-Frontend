...
export default function Presencas(){
    ...
    return (
        ...
        <main style={{padding: "6rem 0", alignItems: "stretch", minHeight: "50vh"}}>
            ...
            <Link className="border rounded-lg p-4 absolute top-34 right-8 hover:bg-blue-400 transition duration-900" to="/presencas/register">
                <FaTasks /> Registrar
            </Link>
            {users.map((user) => (
                <article key={user.id} className="w-4/10 min-h-16 flex flex-col border border-gray-400 rounded-lg p-4 ">
                    ...
                    <div className="flex-2 w-full flex items-center justify-center px-4 py-2">
                        <div className="flex-1 h-9/10 flex items-center justify-center rounded-full overflow-hidden border-2 border-gray-300 text-center">
                            {user.image ? 
                            (<img src={`http://127.0.0.1:8000/storage/${user.image}`} alt="Perfil" className="w-full h-full object-cover"/>
                            ) 
                            : 
                            (<FaUser className="text-blue-500 text-7xl" />)}
                        </div>
