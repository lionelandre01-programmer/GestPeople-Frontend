...
export default function PresencasRegist(){
    ...
    const [horaEnt, setHoraEnt] = useState('08:00');
    const [horaSai, setHoraSai] = useState('17:00');
    ...
    function getData(e){
        ...
        if (userId && presente && justificada){
            ...
            if (presente > 0 && parseInt(horaEnt.split(':')[0]) > 8){
    ...
    return (
        <>
            ...
            <main style={{padding: "6rem 0"}}>
                ...
                {processamento ? <Processamento text="Registrando..." /> : ""}
                ...
                <div className="w-full min-h-20 flex flex-col items-center justify-center">
                    ...
                    (<div className="h-40 w-6/10 flex border-b-0 border border-gray-500 rounded-lg p-2">
                        ...
                        <div className="flex-2 h-full pl-6">
                            ...
                            <p>Efectividade: <span className="text-gray-500">{user?.ult_suspensao.efectivo ? "Efectivo" : "Não Efectivo"}</span></p>
                    ...
                    <form className="flex-1 w-6/10 flex flex-col border border-t-0 border-mist-700 rounded-lg" onSubmit={getData}>
                        ...
                        <div className="flex-2 w-full flex px-4 flex-wrap text-2xl font-bold">
                            <div className="flex-1 w-full flex flex-col">
                                <div className="flex-1 w-full flex pr-20">
                                    <label htmlFor="present" className="flex-1">Presente</label>
                                    <input type="radio" id="present" name="presente" value="1" className="border border-blue-300 rounded-lg w-6" 
                                    onChange={(e) => setPresente(e.target.value)}/>
                                </div>
                                
                                <div className="flex-1 w-full flex pr-20">
                                    <label htmlFor="ausent" className="flex-1">Ausente</label>
                                    <input type="radio" id="ausent" name="presente" value="0" className="w-6 border border-blue-300 rounded-lg" 
                                    onChange={(e) => setPresente(e.target.value)}/>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 w-full flex flex-col">
                                <div className="flex-1 w-full flex pr-20">
                                    ...
                                    <input type="radio" id="justifyed" name="justificada" value="1" className="w-6 border border-blue-300 rounded-lg" 
                                    onChange={(e) => setJustificada(e.target.value)}/>
                                </div>
                                
                                <div className="flex-1 w-full flex pr-20">
                                    <label htmlFor="notJustifyed" className="flex-1">Não Justificada</label>
                                    <input type="radio" id="notJustifyed" name="justificada" value="0" className="w-6 border border-blue-300 rounded-lg" 
                                    onChange={(e) => setJustificada(e.target.value)}/>
                                </div>
                            </div>

                            <div className="flex-1 min-w-[50vw] flex mt-5">
                                <div className="flex-1 w-full flex pr-20">
                                    <label htmlFor="ent" className="flex-1">Hora De Entrada</label>
                                    <input type="time" id="ent" value={horaEnt} className="w-[50%] h-10 px-3 border border-gray-700 rounded-lg" 
                                    onChange={(e) => setHoraEnt(e.target.value)}/>
                                </div>
                                
                                <div className="flex-1 w-full flex pr-20">
                                    <label htmlFor="sai" className="flex-1">Hora De Saída</label>
                                    <input type="time" id="sai" value={horaSai} className="w-[50%] h-10 px-3 border border-blue-300 rounded-lg" 
                                    onChange={(e) => setHoraSai(e.target.value)}/>
                                </div>
                            </div>

                            <div className="min-w-[50%] flex mt-5 gap-4 justify-evenly">
                                    <label htmlFor="dat">Data</label>
                                    <input type="date" id="dat" value={day} className="h-10 px-3 border border-gray-700 rounded-lg" 
